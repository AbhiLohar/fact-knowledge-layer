...
const RelationsPage = () => {
  ...
  // Filter based on active tab and live search query
  const filteredRelations = useMemo(() => {
    ...
    if (search && search.trim()) {
      ...
      list = list.filter(r => {
        ...
        const relType = (r.relation_type || '').toLowerCase();
        const stmtA = (r.fact_a?.statement || '').toLowerCase();
        const stmtB = (r.fact_b?.statement || '').toLowerCase();
        const quoteA = (r.fact_a?.source_quote || '').toLowerCase();
        const quoteB = (r.fact_b?.source_quote || '').toLowerCase();
        const docA = (r.fact_a?.document_name || '').toLowerCase();
        const docB = (r.fact_b?.document_name || '').toLowerCase();
        const timeA = (r.fact_a?.time_context || '').toLowerCase();
        const timeB = (r.fact_b?.time_context || '').toLowerCase();
  ...
  return (
    <div className="space-y-6 h-full flex flex-col max-w-5xl">
      ...
      {/* Segmented Tabs & Live Search Bar */}
      <div className="flex flex-col md:flex-row gap-3 items-start md:items-center justify-between">
        ...
        <div className="w-full md:w-72">
          <SearchBar onSearch={setSearch} placeholder="Search relations & evidence..." />
        </div>
      </div>
